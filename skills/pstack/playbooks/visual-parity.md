...
4. Verify each component against its baseline via image diff on the matching surface on the matching real surface. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `long-runlong-run/loop` per component until the diff is zero.
