...
- `model`: your configured why-investigators model (default `model_role:fast_explore` / feature_impl (Cursor example: grok-4.5-fast-xhigh))
...
- adapter `explore` / `implement` helper
- `model`: your configured why-synthesizer model (default `model_role:judgment` (Cursor example: claude-fable-5-thinking-max))
